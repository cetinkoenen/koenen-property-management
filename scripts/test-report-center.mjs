...
const modules=buildReportCenter(input);assert.equal(modules.length,17);
...
assert.equal(rentalHistoryReport.find(m=>m.id==='eur').tables[0].rows.find(row=>row[0]==='Kaltmiete')?.[3],'540,00 €','Historische Gesamtmiete muss über den passenden Vermietungszeitraum in Kaltmiete aufgeteilt werden');
assert.equal(rentalHistoryReport.find(m=>m.id==='eur').tables[0].rows.find(row=>row[0]==='Nebenkostenzahlungen')?.[3],'80,00 €');
const empty=buildReportCenter({...input,entries:[],sources:{},rent:null});assert.equal(empty.length,17);assert.equal(empty.find(m=>m.id==='journal').tables[0].rows.length,0);
