...
assert.deepEqual(module('wealth-statement').tables[0].headers,['Objektdaten','Aktueller Bestand'],'SSOT-Referenzen dürfen im sichtbaren Bericht nicht als dritte Spalte erscheinen');
assert.equal(module('wealth-statement').tables[0].sourceReferences.find(reference=>reference.field==='Gesamte Wohnfläche')?.source,'Immobilienvermögen · property_extra_info','Die unsichtbare SSOT-Referenz muss intern je Feld erhalten bleiben');
...
assert.doesNotMatch(module('wealth-statement').paragraphs.join(' '),/property_extra_info|tenant_contracts|Single Source of Truth/,'Technische Datenbankreferenzen dürfen im professionellen Bericht nicht sichtbar sein');
