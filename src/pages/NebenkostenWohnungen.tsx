...
type BuildingMeta = { propertyCode: string; propertyLabel: string; billingYear: number; periodFrom: string; periodTo: string; landlordName: string; landlordAddress: string; attachmentReferences: string; workflowStatus: NkWorkflowStatus; locked: boolean; settlementStatus?: "Offen" | "Ausgeglichen"; settlementDate?: string; settlementAmount?: number; settlementEntryId?: number; settlementReference?: string };
...
export default function NebenkostenWohnungen() {
  ...
  function exportOnePager() {
    ...
    const lines = [
      ...
      ...(workspace.meta.settlementStatus === "Ausgeglichen" ? [
        `Zahlungsstatus: Ausgeglichen am ${formatDate(workspace.meta.settlementDate || "")}`,
        `Zahlbetrag: ${formatCurrency(workspace.meta.settlementAmount ?? Math.abs(tenantBalance))}`,
        `Buchungsreferenz: ${workspace.meta.settlementReference || `Buchung ${workspace.meta.settlementEntryId ?? "—"}`}`,
      ] : []),
      "",
      "ANLAGEN / REFERENZEN",
      ...numberedAttachments,
...
<body>
  <section class="page">
    ...
    ${workspace.meta.settlementStatus === "Ausgeglichen" ? `<div class="box small"><strong>Zahlungsstatus: Ausgeglichen</strong> · Zahlung ${escapeHtml(formatCurrency(workspace.meta.settlementAmount ?? Math.abs(tenantBalance)))} am ${escapeHtml(formatDate(workspace.meta.settlementDate || ""))} · ${escapeHtml(workspace.meta.settlementReference || `Buchung ${workspace.meta.settlementEntryId ?? "—"}`)}</div>` : ""}
